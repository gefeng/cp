...
public:
    ...
    int minTimeToReach(std::vector<std::vector<int>>& moveTime) {
        ...
        std::vector<std::vector<std::vector<int64_t>>> best(1, std::vector<std::vector<int64_t>>(n, std::vector<int64_t>(m, INF)));
        ...
        while(!pq.empty()) {
            ...
            for(int d = 0; d < 4; d++) {
                ...
                if(nr >= 0 && nc >= 0 && nr < n && nc < m) {
                    int64_t nt = std::max(t, int64_t(moveTime[nr][nc])) + 1;
                    if(nt < best[nf][nr][nc]) {
                        best[nf][nr][nc] = nt;
                        pq.push({nt, nr, nc, nf});
                    }
